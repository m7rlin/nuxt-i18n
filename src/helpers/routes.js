...
exports.makeRoutes = (
  ...
  const buildLocalizedRoutes = (
    ...
    // Disabled pages
    const disabledPages = ["id-story"]

    // Skip if page shouldn't be multilangual
    ...
    if (
      ...
      componentOptions.locales = componentOptions.locales.filter((locale) =>
        pageOptions.locales.includes(locale),
      )
    }
    ...
    for (
      ...
      if (componentOptions.paths && componentOptions.paths[locale]) {
        path = encodePaths
          ? encodeURI(componentOptions.paths[locale])
          : componentOptions.paths[locale]
      }
